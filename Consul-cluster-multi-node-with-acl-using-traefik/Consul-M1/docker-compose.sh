...
TRAEFIK_TOKEN="your_uuid_traefik token"
...
sleep 5
echo "$(date +"%Y-%m-%d %H:%M:%S"): Configure Traefik ACL"
docker exec -e CONSUL_HTTP_TOKEN="${MASTER_TOKEN}" -it "${CONSUL_HOST}" consul acl policy create -name traefik-policy -description "Policy capable of comunicate traefik app with consul" -rules @/tmp/hcl/traefik-policy.hcl >/dev/null
docker exec -e CONSUL_HTTP_TOKEN="${MASTER_TOKEN}" -it "${CONSUL_HOST}" consul acl token create -description "Traefik App" -policy-name traefik-policy -secret="${TRAEFIK_TOKEN}" >/dev/null
docker exec -e CONSUL_HTTP_TOKEN="${MASTER_TOKEN}" -it "${CONSUL_HOST}" consul acl set-agent-token agent "${TRAEFIK_TOKEN}" >/dev/null
